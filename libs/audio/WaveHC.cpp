 ...
 * William Greiman has made many changes that may have introduced bugs. Major changes are:
...
uint8_t WaveHC::create(FatReader &f) {
  ...
  if (Channels > 2) {
    debug("Not mono/stereo!");
    return false;
  }
  else if (Channels > 1) {
    debug(" Warning stereo file!");
  }
  
  BitsPerSample = buf.fmt.bitsPerSample;
  if (BitsPerSample > 16) {
    debug("More than 16 bits per sample!");
    return false;
  }
